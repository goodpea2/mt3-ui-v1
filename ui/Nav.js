    ...
    <div class="absolute bottom-0 left-0 w-full h-20 bg-gradient-to-t from-black via-[#0c051d] to-transparent pointer-events-none z-50">
      <div class="absolute bottom-4 left-0 w-full flex justify-around items-end px-4 pointer-events-auto">
        <div class="relative cursor-pointer group">
          <div class="absolute inset-x-[-10px] top-[-15px] bottom-[-3px] bg-gradient-to-b from-cyan-400 to-blue-600 rounded-xl shadow-2xl opacity-80 group-hover:scale-105 transition-transform"></div>
          <div class="relative bg-gradient-to-b from-[#ff4081] to-[#e91e63] w-12 h-12 rounded-xl flex flex-col items-center justify-center border-2 border-white/40 shadow-inner">
             ...
             <span class="text-white text-[8px] font-black italic tracking-wider uppercase">HOME</span>
          </div>
        </div>
        ${['Library', 'Playlist', 'Rank', 'Shop'].map((label) => `
          <div class="mb-1 opacity-60 hover:opacity-100 transition-all cursor-pointer flex flex-col items-center transform hover:scale-110">
             ...
             <span class="text-[7px] font-black text-purple-300 mt-0.5 uppercase">${label}</span>
          </div>
        `).join('')}
      </div>
    </div>
